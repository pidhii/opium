...
TEST(LispReaderTest, MultipleExpressions) {
  ...
  ASSERT_TRUE(opi::ispair(result1));
  ASSERT_TRUE(opi::isnum(opi::car(result1)));
  ASSERT_EQ(opi::num_val(opi::car(result1)), 1.0);
  
  // Extract second expression
  opi::value result2 = opi::nil;
  ASSERT_TRUE(reader >> result2);
  ASSERT_TRUE(opi::ispair(result2));
  ASSERT_TRUE(opi::isnum(opi::car(result2)));
  ASSERT_EQ(opi::num_val(opi::car(result2)), 3.0);
...
TEST(LispReaderTest, MultiplePartialInputs) {
  ...
  ASSERT_TRUE(opi::ispair(result));
  ASSERT_TRUE(opi::isnum(opi::car(result)));
  ASSERT_EQ(opi::num_val(opi::car(result)), 3.0);
...
TEST(LispReaderTest, DifferentValueTypes) {
  ...
  ASSERT_TRUE(reader >> result);
  ASSERT_TRUE(opi::isbool(result));
  ASSERT_EQ(result, opi::True);
  
  // Extract and verify nil
  ASSERT_TRUE(reader >> result);
  ASSERT_EQ(result, opi::nil);
