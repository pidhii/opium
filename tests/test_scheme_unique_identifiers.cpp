...
protected:
    ...
    opi::value parse(const std::string& code) {
        return opi::lisp_parser{}.parse(code);
    }

    // Helper function to check if a symbol has the expected format
    bool is_unique_symbol(const opi::value& val, const std::string& prefix) {
...
TEST_F(SchemeUniqueIdentifiersTest, LambdaVariableReplacement) {
    ...
    EXPECT_NE(opi::sym_name(param_x), opi::sym_name(param_y)); // Different symbols
...
TEST_F(SchemeUniqueIdentifiersTest, LetrecVariableReplacement) {
    ...
    opi::value expr = parse("(letrec ((fact (lambda (n) (if (= n 0) 1 (* n (fact (- n 1))))))) (fact 5))");
    opi::value result = (*transformer)(expr);
...
TEST_F(SchemeUniqueIdentifiersTest, FunctionDefineReplacement) {
    ...
    EXPECT_TRUE(is_unique_symbol(param_x, "_test"));
    EXPECT_NE(opi::sym_name(func_name), opi::sym_name(param_x)); // Different symbols
...
TEST_F(SchemeUniqueIdentifiersTest, NestedScopes) {
    ...
    EXPECT_TRUE(opi::equal(inner_body_x, inner_var_x)); // Same symbol as inner binding
    EXPECT_FALSE(opi::equal(inner_body_x, outer_var_x)); // Not the same as outer binding
}
