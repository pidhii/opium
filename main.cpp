...
main(int argc, char **argv)
{
  ...
  try {
    ...
    size_t cnt = 0;
    const auto [result, out, type_map] = translate_to_scheme(cnt, pl, in);

    // Display final Prolog expression generated by type-checker
    for (const value outexpr : range(out))
      std::cout << pprint_scm(outexpr) << std::endl << std::endl;
    std::cout << std::endl;
    std::cout << std::endl;

    // Display the source file with type annotations
    std::cout << "\n\e[1mSource File with Type Annotations:\e[0m" << std::endl;

    std::ifstream infile {"./test.scm"};
    type_map.display_source_with_types(infile, std::cout);
  }
